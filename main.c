...
void update_bird(Bird *bird) {
    ...
    if (bird->y > SCREEN_HEIGHT - GROUND_HEIGHT - 20) {
        bird->y = SCREEN_HEIGHT - GROUND_HEIGHT - 20;
        bird->velocity = 0;
    }
}
...
void update_pipes(Pipe *top_pipe, Pipe *bottom_pipe, int *score) {
    ...
    if (top_pipe->x + PIPE_WIDTH < 0) {
        int gap = rand() % (MAX_GAP - MIN_GAP) + MIN_GAP;
        top_pipe->height = rand() % (SCREEN_HEIGHT - gap);
        bottom_pipe->height = top_pipe->height + gap;
